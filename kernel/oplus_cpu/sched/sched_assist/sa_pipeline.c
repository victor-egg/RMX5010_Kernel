...
#define ALLOWED_TOP_APP_TASK_DEFAULT_UTIL 300
#define ALLOWED_LOW_LATENCY_TASK_DEFAULT_UTIL 30
...
bool oplus_pipeline_low_latency_task(int pipeline_cpu)
{
	...
	if (task->prio < MAX_RT_PRIO) {
		if (prime_cpu_num <= 2 && pipeline_cpu >= nr_cpu_ids - prime_cpu_num) {
			return false;
		} else {
			...
			if (rt_util < allowed_low_latency_task_util)
				return false;
			else
				return true;
		}
	...
	/* audio task */
	if ((ots->ux_state & SA_TYPE_SWIFT) && !(ots->ux_state & SA_TYPE_INHERIT)) {
		if (prime_cpu_num <= 2 && pipeline_cpu >= nr_cpu_ids - prime_cpu_num) {
			return false;
		} else {
			...
			if (audio_util < allowed_low_latency_task_util)
				return false;
			else
				return true;
		}
...
bool oplus_pipeline_task_skip_cpu(struct task_struct *task, unsigned int dst_cpu)
{
	...
	/* rt task */
	if (task->prio < MAX_RT_PRIO) {
		if (oplus_pipeline_rt_skip_prime_cpu(dst_cpu))
			return true;
	}

	/* try migrating prime_task from prime_cpu to gold_cpu or silver_cpu */
	if (task == prime_task &&
		task_cpu(task) >= nr_cpu_ids - prime_cpu_num &&
		dst_cpu < nr_cpu_ids - prime_cpu_num) {
		if (unlikely(global_debug_enabled & DEBUG_PIPELINE)) {
			char buf[64];

			snprintf(buf, sizeof(buf), "%s_%d_m_refused\n", task->comm, task->pid);
	...
	if (dst_cpu < nr_cpu_ids - prime_cpu_num)
		return false;
	...
	if (!is_pipeline_task) {
		...
		/* other top app ui and render tasks can run on prime cpus */
		if ((ots->ux_state & SCHED_ASSIST_UX_PRIORITY_MASK) > UX_PRIORITY_PIPELINE) {
			if ((task->tgid != prime_tgid) && task_util(task) > ALLOWED_TOP_APP_TASK_DEFAULT_UTIL)
				return false;
		} else {
...
EXPORT_SYMBOL_GPL(oplus_pipeline_prime_cpu_capacity_fit);

static unsigned long rt_skip_cpumask_val = 0;
static cpumask_t rt_skip_cpumask = { CPU_BITS_NONE };
bool oplus_pipeline_rt_skip_prime_cpu(unsigned int dst_cpu)
{
	...
	if (rt_skip_cpumask_val > 0) {
		if (cpumask_test_cpu(dst_cpu, &rt_skip_cpumask))
			skip = true;
	} else {
		if ((dst_cpu == nr_cpu_ids - 1) ||
			(dst_cpu == nr_cpu_ids - 2 && prime_cpu_num == 2) ||
			(dst_cpu == rt_skip_extra_pipeline_cpu && nr_cpu_ids >= 8))
			skip = true;
	}

	if (skip && unlikely(global_debug_enabled & DEBUG_PIPELINE)) {
		char buf[64];

		snprintf(buf, sizeof(buf), "rt_skip_cpu%d\n", dst_cpu);
		systrace_c_printk(buf, 1);
		systrace_c_printk(buf, 0);
	}
			...
			if (task) {
				...
				if (!IS_ERR_OR_NULL(ots)) {
					...
					if (cpus[i] == nr_cpu_ids - 1) {
						...
						if (cpus[i] > 0 && cpus[i] > rt_skip_extra_pipeline_cpu)
							rt_skip_extra_pipeline_cpu = cpus[i];
					}
...
static ssize_t pipeline_rt_cpumask_proc_write(struct file *file,
			const char __user *buf, size_t count, loff_t *ppos)
{
	char buffer[32] = {0};
	int err, val;
	...
	rt_skip_cpumask_val = (unsigned long)val;

	if (rt_skip_cpumask_val > 0) {
		cpumask_copy(&rt_skip_cpumask, to_cpumask(&rt_skip_cpumask_val));
		/* sanity check */
		if (cpumask_weight(&rt_skip_cpumask) > 4) {
			rt_skip_cpumask_val = 0;
			cpumask_clear(&rt_skip_cpumask);
		}
	} else {
		cpumask_clear(&rt_skip_cpumask);
	}
...
static ssize_t pipeline_rt_cpumask_proc_read(struct file *file,
			char __user *buf, size_t count, loff_t *ppos)
{
	char buffer[32] = {0};
	int len;

	len = snprintf(buffer, sizeof(buffer), "0x%lx\n", rt_skip_cpumask_val);

	return simple_read_from_buffer(buf, count, ppos, buffer, len);
}

static const struct proc_ops pipeline_rt_cpumask_proc_ops = {
	.proc_write		= pipeline_rt_cpumask_proc_write,
	.proc_read		= pipeline_rt_cpumask_proc_read,
	.proc_lseek		= default_llseek,
};
