...
static unsigned int soft_freq_clamp(struct scx_gov_policy *sg_policy, unsigned int target_freq)
{
	...
	target_freq = max(sg_policy->boost_min_freq, target_freq);
...
void scx_gov_boost_max(struct scx_gov_policy *sg_policy, int boost)
{
	unsigned int next_f;
	struct cpufreq_policy *policy = sg_policy->policy;
	unsigned long irq_flags;
	...
	if (!boost) {
		sg_policy->boost_min_freq = 0;
		next_f = soft_freq_clamp(sg_policy, sg_policy->next_freq);
		next_f = cpufreq_driver_resolve_freq(policy, next_f);
	} else {
		sg_policy->boost_min_freq = policy->cpuinfo.max_freq;
...
void scx_boost_or_reset_all(int boost)
{
	...
	for_each_sched_cluster(cluster) {
		if (!gov_flag[cluster->id])
			continue;
		policy = cpufreq_cpu_get_raw(cpumask_first(&cluster->cpus));
		scx_gov_boost_max(policy->governor_data, boost);
	}
	rcu_read_unlock();
}
...
static void scx_boost_update_cpufreq(struct update_util_data *update_util, u64 time, unsigned int flags)
{
	...
	if ((flags & HMBIRD_TICK_HIT_BOOST)) {
		...
		scx_boost_systrace_c(tick_hit_boost_cpu, next_f);
		if (policy->fast_switch_enabled)
			cpufreq_driver_fast_switch(policy, next_f);
		else
			kthread_queue_work(&sg_policy->worker, &sg_policy->work);
