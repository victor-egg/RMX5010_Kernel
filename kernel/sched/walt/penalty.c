...
void penalty_flush_task(struct task_struct *p){
	flush_task(p);
}
EXPORT_SYMBOL_GPL(penalty_flush_task);

static void __maybe_unused android_rvh_before_do_sched_yield(void *unused, long *skip)
{
	u64 delta_total;
	u64 clock;
	u64 frame_time_ns_;
...
#if !IS_ENABLED(CONFIG_OPLUS_YIELD_OPT)
