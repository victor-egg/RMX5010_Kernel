...
static int gcc_debug_mux_sels[] = {
	...
	0x1E6,		/* gcc_pcie_1_aux_clk */
	0x1E7,		/* gcc_pcie_1_cfg_ahb_clk */
	0x1E8,		/* gcc_pcie_1_mstr_axi_clk */
	0x1E9,		/* gcc_pcie_1_phy_rchng_clk */
	0x1EA,		/* gcc_pcie_1_pipe_clk */
	0x1EB,		/* gcc_pcie_1_pipe_div2_clk */
	0x1EC,		/* gcc_pcie_1_slv_axi_clk */
	0x1ED,		/* gcc_pcie_1_slv_q2a_axi_clk */
	0xF3,		/* gcc_pdm2_clk */
	...
	0x1EF,		/* gcc_sdcc1_apps_clk */
	0x1F1,		/* gcc_sdcc1_ice_core_clk */
...
static const char *const gpu_cc_debug_mux_parent_names[] = {
	...
	"gpu_cc_sleep_clk",
	"measure_only_gpu_cc_acd_ahb_clk",
	...
	"measure_only_gpu_cc_gx_accu_shift_clk",
...
static int gpu_cc_debug_mux_sels[] = {
	...
	0x2B,		/* measure_only_gpu_cc_acd_ahb_clk */
	...
	0x2E,		/* measure_only_gpu_cc_gx_accu_shift_clk */
...
static struct clk_dummy measure_only_gpu_cc_acd_ahb_clk = {
	.rrate = 1000,
	.hw.init = &(const struct clk_init_data){
		.name = "measure_only_gpu_cc_acd_ahb_clk",
		.ops = &clk_dummy_ops,
	},
};
...
static struct clk_dummy measure_only_gpu_cc_gx_accu_shift_clk = {
	.rrate = 1000,
	.hw.init = &(const struct clk_init_data){
		.name = "measure_only_gpu_cc_gx_accu_shift_clk",
		.ops = &clk_dummy_ops,
	},
};
...
static struct clk_hw *debugcc_kera_hws[] = {
	...
	&measure_only_gpu_cc_gx_accu_shift_clk.hw,
...
static int clk_debug_kera_probe(struct platform_device *pdev)
{
	struct clk *clk;
	int ret = 0, i;
	...
	for (i = 0; i < ARRAY_SIZE(debugcc_kera_hws); i++) {
		clk = devm_clk_register(&pdev->dev, debugcc_kera_hws[i]);
		if (IS_ERR(clk)) {
			dev_err(&pdev->dev, "Unable to register %s, err:(%ld)\n",
				qcom_clk_hw_get_name(debugcc_kera_hws[i]),
				PTR_ERR(clk));
			return PTR_ERR(clk);
		}
	}
...
static int __init clk_debug_kera_init(void)
{
	return platform_driver_register(&clk_debug_driver);
}
fs_initcall(clk_debug_kera_init);

MODULE_DESCRIPTION("QTI DEBUG CC KERA Driver");
MODULE_LICENSE("GPL");
