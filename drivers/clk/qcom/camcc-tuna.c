...
static struct clk_vdd_class *cam_cc_tuna_regulators[] = {
	&vdd_mm,
	&vdd_mx,
	&vdd_mxc,
};

static struct clk_vdd_class *cam_cc_tuna_regulators_1[] = {
	&vdd_mm,
	&vdd_mxc,
};
...
static struct clk_rcg2 cam_cc_camnoc_rt_axi_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = cam_cc_tuna_regulators_1,
		.num_vdd_classes = ARRAY_SIZE(cam_cc_tuna_regulators_1),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
...
static struct clk_rcg2 cam_cc_cphy_rx_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = cam_cc_tuna_regulators,
		.num_vdd_classes = ARRAY_SIZE(cam_cc_tuna_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
...
static const struct freq_tbl ftbl_cam_cc_ipe_nps_clk_src[] = {
	F(309000000, P_CAM_CC_PLL1_OUT_EVEN, 1, 0, 0),
...
static const struct freq_tbl ftbl_cam_cc_ofe_clk_src[] = {
	F(283000000, P_CAM_CC_PLL2_OUT_EVEN, 1, 0, 0),
...
static const struct freq_tbl ftbl_cam_cc_tfe_0_clk_src[] = {
	F(289000000, P_CAM_CC_PLL3_OUT_EVEN, 1, 0, 0),
...
static const struct freq_tbl ftbl_cam_cc_tfe_1_clk_src[] = {
	F(289000000, P_CAM_CC_PLL4_OUT_EVEN, 1, 0, 0),
...
static const struct freq_tbl ftbl_cam_cc_tfe_2_clk_src[] = {
	F(289000000, P_CAM_CC_PLL5_OUT_EVEN, 1, 0, 0),
	F(400000000, P_CAM_CC_PLL5_OUT_EVEN, 1, 0, 0),
	F(525000000, P_CAM_CC_PLL5_OUT_EVEN, 1, 0, 0),
...
static struct clk_regmap *cam_cc_tuna_clocks[] = {
...
static const struct qcom_reset_map cam_cc_tuna_resets[] = {
...
static const struct regmap_config cam_cc_tuna_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static struct qcom_cc_desc cam_cc_tuna_desc = {
	.config = &cam_cc_tuna_regmap_config,
	.clks = cam_cc_tuna_clocks,
	.num_clks = ARRAY_SIZE(cam_cc_tuna_clocks),
	.resets = cam_cc_tuna_resets,
	.num_resets = ARRAY_SIZE(cam_cc_tuna_resets),
	.clk_regulators = cam_cc_tuna_regulators,
	.num_clk_regulators = ARRAY_SIZE(cam_cc_tuna_regulators),
};

static const struct of_device_id cam_cc_tuna_match_table[] = {
	{ .compatible = "qcom,tuna-camcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, cam_cc_tuna_match_table);

static int cam_cc_tuna_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &cam_cc_tuna_desc);
	if (IS_ERR(regmap))
		return PTR_ERR(regmap);

	ret = qcom_cc_runtime_init(pdev, &cam_cc_tuna_desc);
	if (ret)
		return ret;
	...
	ret = qcom_cc_really_probe(pdev, &cam_cc_tuna_desc, regmap);
	if (ret) {
...
static void cam_cc_tuna_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &cam_cc_tuna_desc);
}

static const struct dev_pm_ops cam_cc_tuna_pm_ops = {
	SET_RUNTIME_PM_OPS(qcom_cc_runtime_suspend, qcom_cc_runtime_resume, NULL)
	SET_SYSTEM_SLEEP_PM_OPS(pm_runtime_force_suspend,
				pm_runtime_force_resume)
};

static struct platform_driver cam_cc_tuna_driver = {
	.probe = cam_cc_tuna_probe,
	.driver = {
		.name = "cam_cc-tuna",
		.of_match_table = cam_cc_tuna_match_table,
		.sync_state = cam_cc_tuna_sync_state,
		.pm = &cam_cc_tuna_pm_ops,
	},
};

static int __init cam_cc_tuna_init(void)
{
	return platform_driver_register(&cam_cc_tuna_driver);
}
subsys_initcall(cam_cc_tuna_init);

static void __exit cam_cc_tuna_exit(void)
{
	platform_driver_unregister(&cam_cc_tuna_driver);
}
module_exit(cam_cc_tuna_exit);

MODULE_DESCRIPTION("QTI CAM_CC TUNA Driver");
MODULE_LICENSE("GPL");
